...
export async function POST(): Promise<Response> {
  try {
    ...
    if (!runResponse.ok) return Response.json({ error: "LandOps API run request failed" }, { status: runResponse.status });
    const started = await runResponse.json() as { runId?: string };
    if (!started.runId) return Response.json({ error: "LandOps API returned no run identifier" }, { status: 502 });
    ...
    if (!runResponseBody.ok || !evidenceResponse.ok || !caseResponse.ok) return Response.json({ error: "LandOps API returned an incomplete run response" }, { status: 502 });
    ...
    return Response.json({ error: error instanceof Error ? error.message : "LandOps API run request failed" }, { status: 502 });
  }
}
